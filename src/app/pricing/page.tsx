...
export default function PricingPage() {
  ...
  return (
    <main className="pt-[56px] bg-gradient-to-br from-[#0a0f0a] via-[#0f1411] to-[#141a14] min-h-screen">
      ...
      <section className="py-8 px-4 lg:px-8">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-4xl lg:text-5xl font-black mb-4">
            ...
            <span className="text-[#f8f9fa]">Pricing</span>
          </h1>
          <p className="text-lg text-[#e9ecef] mb-8 max-w-3xl mx-auto leading-relaxed">
            Find the perfect plan for your vehicle. All models use a <span className="font-semibold text-[#f8f9fa]">premium GI frame and Oxford 600D fabric.</span>
          </p>

          {/* HIGH CONTRAST PRICING TOGGLE */}
          <div className="flex items-center justify-center gap-4 mb-12">
            <span className={`text-sm font-medium ${!isAnnual ? 'text-[#f8f9fa]' : 'text-[#adb5bd]'}`}>
              One-time
            </span>
            <button
            ...
            {pricingPlans.map((plan, index) => (
              <div
                ...
                className={`relative backdrop-blur-sm border rounded-2xl p-8 transition-all duration-300 ${
                  plan.popular 
                    ? 'bg-gradient-to-b from-[#2d3a2d]/90 to-[#1e2a1e]/95 border-green-400/50 shadow-2xl shadow-green-500/20 ring-2 ring-green-400/30 -translate-y-8 scale-105' 
                    : 'bg-gradient-to-b from-[#2a2f2a]/80 to-[#1f241f]/90 border-gray-500/30 shadow-xl hover:shadow-2xl hover:scale-[1.02] hover:border-gray-400/50'
                }`}
              >
                {/* Popular Badge */}
                {plan.popular && (
                ...
                <div className="text-center mb-8">
                  <h3 className={`font-bold text-[#f8f9fa] mb-2 ${plan.popular ? 'text-2xl' : 'text-xl'}`}>{plan.name}</h3>
                  <p className="text-sm text-[#adb5bd] mb-4">{plan.subtitle}</p>
                  
                  {/* Price */}
                  <div className="mb-6">
                    <div className={`font-black mb-1 ${plan.popular ? 'text-5xl text-green-400' : 'text-4xl text-[#f8f9fa]'}`}>
                      {isAnnual 
                  ...
                  <a
                    ...
                    className="w-full block py-3 px-6 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-500 hover:to-green-400 shadow-2xl shadow-green-500/30 text-white font-bold text-sm rounded-xl transition-all duration-300 hover:scale-105 text-center"
                  >
                    Get Started
                  </a>
                </div>
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-[#e9ecef] leading-relaxed">{feature}</span>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      {/* HIGH VISIBILITY FEATURES SECTION */}
      <section className="px-4 lg:px-8 py-16 bg-gradient-to-br from-[#1a1f1a]/60 to-[#0f1411]/40">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-[#f8f9fa] mb-12">What's Included with Every SafePark Model</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <div key={idx} className="text-center">
                <div className="w-16 h-16 bg-gradient-to-tr from-green-500 to-green-400 rounded-xl flex items-center justify-center mb-4 mx-auto shadow-2xl">
                  <span className="text-2xl">{feature.icon}</span>
                </div>
                <h3 className="text-[#f8f9fa] font-bold text-lg mb-2">{feature.title}</h3>
                <p className="text-[#adb5bd] text-sm">{feature.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-4 lg:px-8 py-16">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-[#f8f9fa] mb-4">Ready to Protect Your Vehicle?</h2>
          <p className="text-[#e9ecef] text-lg mb-10">Get your custom quote today with free consultation</p>
          
          <div className="flex flex-col sm:flex-row gap-6 justify-center items-center max-w-2xl mx-auto">
            <a 
              href="tel:+919600840058" 
              className="group flex items-center justify-center gap-3 w-full sm:flex-1 bg-gradient-to-r from-green-600 to-green-500 hover:from-green-500 hover:to-green-400 text-white px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 shadow-2xl shadow-green-600/30 hover:shadow-green-500/40 hover:scale-[1.02] min-h-[60px]"
            >
            ...
            <a 
              ...
              className="group flex items-center justify-center gap-3 w-full sm:flex-1 bg-[#2a2f2a]/90 backdrop-blur-sm border border-gray-500/50 hover:border-gray-400/70 text-[#f8f9fa] hover:text-white px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 shadow-xl hover:shadow-2xl hover:scale-[1.02] hover:bg-[#2f342f]/90 min-h-[60px]"
            >
              <div className="flex items-center justify-center w-6 h-6">
                <svg 
